...
for irun, chunk in enumerate(pd.read_sql_query("SELECT UserID FROM UserInfo;", engine, chunksize=chunk_size)):
    uids = chunk['UserID']
    #search_temp = pd.read_sql_query("SELECT UserID, count(SearchID) as c_search FROM trainSearchStream WHERE UserID in (" + ",".join(map(str, uids)) + ") GROUP BY UserID;", engine)
    #chunk = pd.merge(chunk, search_temp, how='left', on=['UserID'])

    #TODO add testStream counts
    
    searchInfo_temp = pd.read_sql_query("SELECT UserID, count(SearchID) as uc_search FROM SearchInfo WHERE UserID in (" + ",".join(map(str, uids)) + ") GROUP BY UserID;", engine)
    #userids = chunk['UserID']
    #del searchInfo_temp['UserID']
    chunk = pd.merge(chunk, searchInfo_temp, how='left', on=['UserID'])
    ...
    visit_temp = pd.read_sql_query("SELECT UserID, count(*) as uc_visit FROM VisitsStream WHERE UserID in (" + ",".join(map(str, uids)) + ") GROUP BY UserID;", engine)
    #print visit_temp.keys()
    chunk = pd.merge(chunk, visit_temp, how='left', on=['UserID'])

    phone_temp = pd.read_sql_query("SELECT UserID, count(*) as uc_phone FROM PhoneRequestsStream WHERE UserID in (" + ",".join(map(str, uids)) + ") GROUP BY UserID;", engine)
